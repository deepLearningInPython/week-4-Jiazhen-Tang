...
# Your code here:
# -----------------------------------------------
word_frequencies = {t: tokens.count(t) for t in set(tokens)}
...
def token_counts(string_input: str, k: int = 1) -> dict:
    ...
    toks = tokenize(string_input)
    
    # 2. Use a dictionary comprehension to count and filter
    # Note: tokens.count(token) iterates the list, which is O(N) inside O(N) loop. 
    # For small text strings this is acceptable for the exercise.
    return {token: toks.count(token) for token in set(toks) if toks.count(token) >= k}
...
# Your code here:
# -----------------------------------------------
token_to_id = {t: i for i, t in enumerate(sorted(list(set(tokens))))}

# Expected output: {'dog': 0, 'quick': 1, 'fox': 2, 'the': 3, 'over': 4, 'lazy': 5, 'brown': 6, 'jumps': 7}
# (Note: indices will depend on sort order)
print(token_to_id)
# -----------------------------------------------
